...
class CommunityPropertiesInterface:
    ...
    def calculate_community_properties(self,lineages,from_time):
        ...
        self.species_volatility = {'lineage ' + str(i) : 
                              self.rescaled_detect_invasability(community.t,
                                                                community.y[species_index, :],
                                                                from_time - 500,
                                                                1e-2,
                                                                from_time)
                              for i, community in enumerate(self.ODE_sols.values())}
            
        self.resource_volatility = {'lineage ' + str(i) : 
                               self.rescaled_detect_invasability(community.t,
                                                                 community.y[resource_index, :],
                                                                 from_time - 500,
                                                                 1e-2,
                                                              from_time)
                                for i, community in enumerate(self.ODE_sols.values())}
        ...
        self.species_fluctuations = {'lineage ' + str(i) : 
                                self.fluctuation_coefficient(community.t,
                                                             community.y[species_index, :],
                                                             1e-2,
                                                             from_time) 
                                for i, community in enumerate(self.ODE_sols.values())}
        
        self.resource_fluctuations = {'lineage ' + str(i) : 
                                self.fluctuation_coefficient(community.t,
                                                             community.y[resource_index, :],
                                                             1e-2,
                                                             from_time) 
        ...
        self.species_survival_fraction = {'lineage ' + str(i) : 
                                     self.diversity(community, species_index, 1e-2, from_time)
                                     for i, community in enumerate(self.ODE_sols.values())}
        self.resource_survival_fraction = {'lineage ' + str(i) : 
                                     self.diversity(community, resource_index, 1e-2, from_time)
                                     for i, community in enumerate(self.ODE_sols.values())}
            
            
    def rescaled_detect_invasability(self, simulation_t, simulation_y,
        ...
        fluctuating_species = extant_species[np.logical_not(np.isnan([self.find_normalised_peaks(simulation_y[spec, t_start_index])[0]
    ...
    def fluctuation_coefficient(self, times, dynamics, extinction_threshold, from_time):
    ...
    def diversity(self, data, index, extinction_threshold, from_time):
...
def max_le(model_class, T, initial_conditions, extinction_threshold, model_version,
    ...
    for n in range(int(np.round(T/dt))):
        ...
        if separation_dt > separation_min and separation_dt < separation_max:
            ...
            simulation1 = model_class.simulate_community(np.arange(1), dt, init_cond_func='user supplied',
                                                         model_version = model_version, assign = 'False',
                                                         user_supplied_init_cond = 
                                                         {'species' : original_conditions[:model_class.no_species],
                                                          'resources' : original_conditions[model_class.no_species:]})
            # Simulate the perturbated community trajectory for time = dt
            simulation2 = model_class.simulate_community(np.arange(1), dt, init_cond_func='user supplied',
                                                         model_version = model_version, assign = 'False',
                                                         user_supplied_init_cond = 
                                                         {'species' : perturbed_conditions[:model_class.no_species],
                                                          'resources' : perturbed_conditions[model_class.no_species:]})
