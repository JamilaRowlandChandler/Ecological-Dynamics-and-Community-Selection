...
def species_diversity(model_object,timeframe,extinct_thresh=1e-4):
    ...
    simulations_copy = deepcopy(model_object.ODE_sol)
    ...
    present_species = \
        np.any(simulations_copy.y[:model_object.no_species,indices[0]:indices[1]] > extinct_thresh,
                             axis=1)
...
def detect_invasibility(model_object,t_start,extinct_thresh=1e-4):
    ...
    t_start_index = np.where(model_object.ODE_sol.t >= t_start)[0]
    
    # set baseline_abundance as slightly greater than the migration rate.
    baseline_abundance = model_object.dispersal * 1e2
    
    # identifying extant/surviving species between t_start and the end of simulations
    extant_species = np.any(model_object.ODE_sol.y[:model_object.no_species,t_start_index] > extinct_thresh,axis = 1).nonzero()[0]
    
    # Identify which of the extant species have "fluctuating dynamics".
    fluctuating_species = extant_species[np.logical_not(np.isnan([find_normalised_peaks(model_object.ODE_sol.y[spec,t_start_index])[0] \
    ...
    if fluctuating_species.size > 0:
        ...
        when_fluctuating_species_are_lost = np.nonzero(model_object.ODE_sol.y[fluctuating_species,t_start_index[0]:] \
                                                        < baseline_abundance)
...
def unique_compositions(model_object):
    ...
    all_compositions = np.vstack(list(model_object.final_composition.values()))
...
def community_function_totalled_over_maturation(model_object):
    ...
    summed_abundances = np.sum(model_object.ODE_sol.y[:model_object.no_species,:],axis=1)
    
    community_function = np.sum(np.multiply(model_object.species_contribution_community_function,
                                            summed_abundances))
